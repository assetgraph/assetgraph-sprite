...
describe('spriteBackgroundImages', function () {
    ...
    it('should process two sprites with -sprite-location properties in the group selector', async function () {
        ...
        const cssImageHrefs = pluck(assetGraph.findRelations({type: 'CssImage'}), 'href').sort();
    ...
    it('should get the background-position right when spriting a @2x image', async function () {
        ...
        expect(assetGraph, 'to contain asset', 'Css');
        expect(assetGraph, 'to contain assets', 'Png', 2);
        expect(assetGraph, 'to contain asset', { type: 'Png', devicePixelRatio: 1 });
        expect(assetGraph, 'to contain asset', { type: 'Png', devicePixelRatio: 2 });

        for (const relation of assetGraph.findRelations({ type: 'CssImage', node: { selector: '.regular' } })) {
            expect(relation.to.devicePixelRatio, 'to be', 1);
            expect(getProperties(relation.node, 'background-size'), 'to be empty');
        }

        for (const relation of assetGraph.findRelations({ type: 'CssImage', node: { selector: '.retina' } })) {
